...
static func find_usages(target_uuid: String) -> Array[String]:
	...
	for prog in programs:
		if is_uuid_in_grid(target_uuid, prog.grid):
			var msg = TranslationServer.translate("program_editor.custom_blocks.used.program").format({ "name": prog.name })
			usages.append(msg)
	...
	for blk in blocks:
		...
		if is_uuid_in_grid(target_uuid, blk.grid):
			var msg = TranslationServer.translate("program_editor.custom_blocks.used.block").format({ "name": blk.name })
			usages.append(msg)
