...
@export_group("Debug / Manual Test")
@export var test_connect_top: bool = false: set = test_conn_top
...
func _ready():
	if not background or not border: return
...
func test_conn_top(val):
	test_connect_top = val
	set_connected(Side.TOP, val)
