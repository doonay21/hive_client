...
func open_form(name_p: String = "", description_p: String = "", uuid_p: String = "") -> void:
	...
	if editing_uuid.is_empty():
		title = tr("program_editor.new_program.title")
		save_button.text = tr("program_editor.new_program.create")
	else:
		title = tr("program_editor.custom_blocks.edit")
		save_button.text = tr("program_editor.new_program.save")
...
func on_save_button_pressed() -> void:
	...
	if name_text.is_empty():
		AlertSystem.show_alert(tr("alert_error"), tr("program_editor.new_program.name.required"), Alert.MessageType.ERROR)
		return
	
	if description_text.is_empty():
		AlertSystem.show_alert(tr("alert_error"), tr("program_editor.new_program.description.required"), Alert.MessageType.ERROR)
		return
	
	if name_regex.search(name_text) == null:
		AlertSystem.show_alert(tr("alert_error"), tr("program_editor.new_program.name.regex"), Alert.MessageType.ERROR)
		return
	
	if description_text.length() > 150:
		AlertSystem.show_alert(tr("alert_error"), tr("program_editor.new_program.description.too_long"), Alert.MessageType.ERROR)
		return
	
	if description_regex.search(description_text) == null:
		AlertSystem.show_alert(tr("alert_error"), tr("program_editor.new_program.description.invalid_chars"), Alert.MessageType.ERROR)
		return
	
	if name_text != original_name and not name_unique(name_text):
		AlertSystem.show_alert(tr("alert_error"), tr("program_editor.new_program.name.not_unique"), Alert.MessageType.ERROR)
		return
