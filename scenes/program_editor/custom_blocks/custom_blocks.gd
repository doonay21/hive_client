...
@export var program_editor: BrainEditor
...
func on_new_block_window_create_new_block(block_name: String, block_description: String) -> void:
	...
	if block_model:
		...
		program_editor.create_new_tab(block_model)
		AlertSystem.show_alert(tr("alert_sucess"), tr("program_editor.custom_blocks.created"), Alert.MessageType.SUCCESS)
	else:
		AlertSystem.show_alert(tr("alert_error"), tr("program_editor.custom_blocks.create_error"), Alert.MessageType.ERROR)
...
func attempt_delete_block() -> void:
	...
	if program_editor.is_block_used_in_open_tabs(context_block_uuid):
		var message = tr("program_editor.custom_blocks.used.error").format({ "name": block_model.name }) + "\n\n"
		message += tr("program_editor.custom_blocks.used.in_editor_warning")
	...
	if not usages.is_empty():
		...
		if usages.size() > limit:
			...
			message += tr("program_editor.custom_blocks.used.and_more").format({ "count": remaining })
...
func on_block_details_edited(uuid: String, new_name: String, new_desc: String) -> void:
	...
	if block_model.save():
		...
		program_editor.on_custom_block_updated(uuid, new_name, new_desc)
		AlertSystem.show_alert(tr("alert_sucess"), tr("program_editor.updated"), Alert.MessageType.SUCCESS)
...
func on_delete_confirmed() -> void:
	...
	program_editor.close_tab_by_uuid(context_block_uuid)
	
	AlertSystem.show_alert(tr("alert_sucess"), tr("program_editor.custom_blocks.delete.alert"), Alert.MessageType.SUCCESS)
	context_block_uuid = ""
