...
const TEX_MISSING = preload("res://assets/images/program_editor/block_icons/missing.png")
...
static func create_missing_block_visuals(block: Block) -> void:
	block.block_data = CustomBlockData.new(
		TranslationServer.translate("program_editor.new_program.missing.title"), 
	...
	block.block_data.info_text = TranslationServer.translate("program_editor.new_program.missing.nf").format({ "uuid": block.custom_block_uuid })
