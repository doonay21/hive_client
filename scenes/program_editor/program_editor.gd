...
const BRAIN_GRID_SCENE: PackedScene = preload("res://scenes/program_editor/program_grid/program_grid.tscn")
...
var program_name: String = tr("program_editor.new_program.def_name")
...
func _ready() -> void:
	...
	program_name_label.text = tr("program_editor.program_label").format({ "program_name": program_name })
...
func on_button_save_pressed() -> void:
	if ProgramManager.save_program(self):
		AlertSystem.show_alert(tr("alert_sucess"), tr("program_editor.saved"), Alert.MessageType.SUCCESS)
	else:
		AlertSystem.show_alert(tr("alert_error"), tr("program_editor.save_error"), Alert.MessageType.ERROR)
...
func on_tab_close_pressed(tab_idx: int) -> void:
	if tab_idx == 0:
		AlertSystem.show_alert(tr("alert_warning"), tr("program_editor.close.warning"), Alert.MessageType.WARNING)
		return
